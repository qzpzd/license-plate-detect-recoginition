...
import torch
import numpy as np
from utils import cfg_mnet, py_cpu_nms, decode, decode_landm, PriorBox, cfg_re50
import cv2
...
class license_plate_ocr:
    def __init__(self, confidence_threshold=0.02, top_k=1000, nms_threshold=0.4, keep_top_k=500, vis_thres=0.6):
        ...
        elif args.cfg == 'Resnet50':
            self.net = Retina(cfg=cfg_re50).to(device)
            self.net = load_model(self.net, 'Resnet50_epoch_40.pth', False)
            self.net.eval()
            self.lprnet = plate_recogition()
            self.priorbox = PriorBox(cfg_re50)   
        # self.net.eval()
        # self.lprnet = plate_recogition()
        #self.priorbox = PriorBox(cfg_mnet)
    ...
    def det_rec(self, srcimg):
        ...
        for b in dets:
            ...
            if args.cfg == 'mobilenet0.25':
                #processed1 = cv2.transpose(processed)
                processed1 = processed
            elif args.cfg == 'Resnet50':
                processed1 = cv2.flip(processed, -1)
            result = self.lprnet.rec(processed1)
            cv2.imshow("processed",processed1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', default='mobilenet0.25', type=str, help='model cfg change')
